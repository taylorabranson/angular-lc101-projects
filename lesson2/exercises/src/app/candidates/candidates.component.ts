...
export class CandidatesComponent implements OnInit {
  ...
  addToCrew (candidateForCrew: object) {
    let alreadyInCrew: boolean = false;
    for (let i = 0; i < this.crew.length; i++) {
      if (candidateForCrew[name] === this.crew[i].name) {
        alreadyInCrew = true;
      }
    }
    if (alreadyInCrew === false) {
      this.crew.push(candidateForCrew);
      return true;
    }
  }

  isInCrew (candidateForCrew: object) {
    let alreadyInCrew: boolean = false;
    for (let i = 0; i < this.crew.length; i++) {
      if (candidateForCrew[name] === this.crew[i].name) {
        alreadyInCrew = true;
